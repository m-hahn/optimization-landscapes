...
wals = read.csv("101A.tab", sep="\t") %>% rename(iso_code=wals.code)
...
u = merge(u, wals, by=c("iso_code"), all.x=TRUE) %>% mutate(value = (description != "Obligatory pronouns in subject position"))
#crash()
write.csv(u %>% select(iso_code, Language, value), file="pro_drop.tsv", sep="\t")
