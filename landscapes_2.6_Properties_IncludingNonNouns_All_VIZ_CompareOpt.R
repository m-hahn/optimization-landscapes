...
optim = read.csv("landscapes_2.6_new.R.tsv", sep=" ")

u = merge(u, optim %>% select(Language, Family, OSSameSideSum, OSSameSideTotal, OSSameSide), by=c("Language", "Family"), all=TRUE)
...
library(ggrepel)
library(ggplot2)
plot = ggplot(u, aes(x=OSSameSide, y=objects, color=Family)) + geom_text_repel(aes(label=Language2)) + xlab("Real Subject-Object Symmetry") + ylab("Coexpressed Subjects and Objects") + theme_bw() + theme(legend.position="none", axis.text=element_text(size=14), axis.title=element_text(size=16)) + theme(panel.grid = element_blank())
