...
for x in sentenceToHash:
 #   print(x)
    ...
    subjects = [i for i in range(len(sent)) if sent[i]["dep"] == "nsubj" and sent[sent[i]["head"]-1]["posUni"] == "VERB"]
    objects = [i for i in range(len(sent)) if sent[i]["dep"] == "obj" and sent[sent[i]["head"]-1]["posUni"] == "VERB"]
    verbs_subj = set([sent[x]["head"] for x in subjects])
    verbs_obj =  set([sent[x]["head"] for x in objects])
    union = set(verbs_subj).union(set(verbs_obj))
    intersection = set(verbs_subj).intersection(set(verbs_obj))
    for verb in union:
       matrix["objects"].append(1 if verb in intersection else 0)
