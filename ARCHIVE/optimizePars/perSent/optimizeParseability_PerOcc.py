...
parser.add_argument('--lr_policy', type=float, default=1e-5, dest="lr_policy")
...
parser.add_argument('--lr_parser', type=float, default=0.001, dest="lr_parser")
parser.add_argument('--dropout_rate_parser', type=float, default=0.2, dest="dropout_rate_parser")
parser.add_argument('--beta1', type=float, default=0.9, dest="beta1")
parser.add_argument('--beta2', type=float, default=0.999, dest="beta2")
parser.add_argument('--clip_at_parser', type=float, default=15.0, dest="clip_at_parser")
parser.add_argument('--clip_norm_parser', type=int, default=2, dest="clip_norm_parser")
parser.add_argument('--pos_embedding_size_parser', type=int, default=100, dest="pos_embedding_size_parser")
parser.add_argument('--lstm_layers_parser', type=int, default=2, dest="lstm_layers_parser")
parser.add_argument('--shallow', type=bool, default=True, dest="shallow")
parser.add_argument('--rnn_dim_parser', type=int, default=200, dest="rnn_dim_parser")
parser.add_argument('--bilinearSize', type=int, default=300, dest="bilinearSize")
parser.add_argument('--input_dropoutRate_parser', type=float, default=0.0, dest="input_dropoutRate_parser")
parser.add_argument('--labelMLPDimension', type=int, default=300, dest="labelMLPDimension")
parser.add_argument('--maxNumberEpochs', type=int, default=5, dest="maxNumberEpochs")
     ...
     for sentence in CorpusIteratorFuncHead(args.language,partition).iterator():
      ...
      for line in sentence:
          ...
          if line["coarse_dep"] == "nsubj":
              line["coarse_dep_key"] = "nsubj_"+str(sentenceHash)+"_"+str(line["index"])
          else:
              line["coarse_dep_key"] = line["coarse_dep"]
          dep = line["coarse_dep_key"]
...
BASE_DIR = "manual_output_funchead_parser_fine_2.6_perOcc"
relevantPath = "/u/scr/mhahn/deps/LANDSCAPE/"+BASE_DIR+"/"
...
word_embeddings_parser = torch.nn.Embedding(num_embeddings = vocab_size_lm+3, embedding_dim = 100).cuda()

dropout_parser = nn.Dropout(args.dropout_rate_parser).cuda()

rnn_parser = nn.LSTM(args.pos_embedding_size_parser+100, args.rnn_dim_parser, args.lstm_layers_parser, bidirectional=True, batch_first=True).cuda() 
...
components_parser = [ pos_u_embeddings_parser, word_embeddings_parser, rnn_parser, headRep, depRep, headMLPOut, dependentMLPOut, labelMLP, labelDecoder] 
...
# For embeddings, Pytorch defaults should be good enough.
initrange = 0.01
...
#       batchOrderedLogits = zip(*map(lambda (y,x):orderSentence(x, dhLogits, y==0 and printHere), zip(range(len(current)),current)))
       ...
       if True:
           ...
           words_layer_parser = word_embeddings_parser(Variable(torch.LongTensor(input_words).transpose(0,1)).cuda())
           inputEmbeddings_parser = torch.cat([pos_u_layer_parser, words_layer_parser], dim=2) # words_layer, 
           if doDropout_parser:
           ...
           for j in range(args.batchSize):
             ...
             policyGradientLoss_parser += lossForThisSentenceMinusBaselines_parser
           policyGradientLoss_parser = float(policyGradientLoss_parser) * overallLogprobSum
           for j in range(args.batchSize):
             for i in range(1,len(batchOrdered[j])+1):
