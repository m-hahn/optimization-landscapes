...
optim = torch.optim.SGD([eta, sigma, xmean2, eta2, sigma2], lr=0.005)
...
#    print((xmean2, [x[1][1][1] for x in observations]))
    ...
    likelihood += - eta2_ / sigma2.pow(2) * sum(((x[1][1][1] - xmean2) - (x[0][1][1] - xmean2) * torch.exp(-eta2_*(x[1][0]-x[0][0]))).pow(2)/(1-torch.exp(-2*eta2_*((x[1][0]-x[0][0])))) for x in observations)
