...
def orderSentence(sentence, dhLogits, printThings, dhWeights, distanceWeights):
   ...
   for line in sentence:
      ...
      if line["coarse_dep"].startswith("punct"): # assumes that punctuation does not have non-punctuation dependents!
         continue
      posHead = sentence[line["head"]-1]["posUni"]
      key = (posHead, line["coarse_dep"], line["posUni"])
...
for step in range(1,n_steps):
    ...
    if step % 2000 == 0:
       print "Saving"
       with open("/u/scr/mhahn/deps/LANDSCAPE/mle-fine-pos/"+language+"_"+__file__+"_model_"+str(myID)+".tsv", "w") as outFile:
          print >> outFile, "\t".join(map(str,["FileName","ModelName","Counter", "AverageLoss","DH_Mean_NoPunct","DH_Sigma_NoPunct", "Distance_Mean_NoPunct", "Distance_Sigma_NoPunct", "Dependency","HeadPOS", "DependentPOS"]))
          ...
          for i in range(len(itos_deps)):
             ...
             print >> outFile, "\t".join(map(str,[myID, __file__, counter, crossEntropy, dh_numpy[i], dh_sigma_numpy[i], dist_numpy[i], dist_sigma_numpy[i], dependency, head, dependent]))
