...
def initializeOrderTable():
   ...
   for partition in ["together"]:
     for sentence in CorpusIterator(language,partition,size=size,shuffleDataSeed=50).iterator():
...
counter = 0
corpus = CorpusIterator(language,"together",size=size,shuffleDataSeed=50)
...
def model(corpus):
  ...
  for q in pyro.irange("data_loop", corpus.length(), subsample_size=5, use_cuda=False):
       ...
       if printHere:
         print "BACKWARD 3 "+__file__+" "+language+" "+str(size)+" "+str(myID)+" "+str(counter)
...
for step in range(1,n_steps):
    ...
    if step % 2000 == 0:
       print "Saving"
       with open("/u/scr/mhahn/deps/LANDSCAPE/mle-fine-size/"+language+"-"+str(size)+"_"+__file__+"_model_"+str(myID)+".tsv", "w") as outFile:
