...
perSentence = read.csv("optimizeDLM/perSentence/outputs/arguments.py.tsv", sep="\t")
...
u = u %>% mutate(Total = O+S+SO) %>% mutate(O=O/Total, S=S/Total, SO=SO/Total)
u = u %>% mutate(TotalN = ON+SN+SON) %>% mutate(ON=ON/TotalN, SN=SN/TotalN, SON=SON/TotalN)

summary(lmer(OSSameSide_Real_Prob ~ SO + (1+SO|Family), data=u))

plot = ggplot(u, aes(x=OSSameSide_Real_Prob, y=SO, color=Family)) + geom_label(aes(label=Language)) 
cor.test(u$OSSameSide_Real_Prob, u$SO)
