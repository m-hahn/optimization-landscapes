...
def recursivelyLinearizeFast(sentence, position, result, gradients_from_the_left_sum):
   line = sentence[position-1]
   if "children_DH" in line:
      for child in line["children_DH"]:
         recursivelyLinearizeFast(sentence, child, result, None)
...
def orderChildrenRelativeFast(sentence, remainingChildren, reverseSoftmax):
       childrenLinearized = []
       while len(remainingChildren) > 0:
           logits = [exp(float(distanceWeightsList[stoi_deps[sentence[x-1]["dependency_key"]]])) for x in remainingChildren]
           s = sum(logits)
           softmax = [x/s for x in logits]
           accum = 0
           sampled = random()
...
def orderSentenceFast(sentence, dhLogits, printThings):
   root = None
   ...
   for line in sentence:
      ...
      dhLogit = float(dhWeightsList[stoi_deps[key]])
   ...
   for line in sentence:
      if "children_DH" in line:
         childrenLinearized = orderChildrenRelativeFast(sentence, line["children_DH"][:], False)
         line["children_DH"] = childrenLinearized
      if "children_HD" in line:
         childrenLinearized = orderChildrenRelativeFast(sentence, line["children_HD"][:], True)
         line["children_HD"] = childrenLinearized
   linearized = []
   overallLogprobSum = recursivelyLinearizeFast(sentence, root, linearized, Variable(torch.FloatTensor([0.0])))
   return linearized
...
def orderSentence(sentence, dhLogits, printThings):
   ...
   for line in sentence:
      ...
      line["children_DH"] = []
      if "linearization_logprobability" in line:
         del line["linearization_logprobability"]
      line["children_decisions_logprobs"] = []

   probabilitiesPositive = 1/(1+torch.exp(-dhWeights))
   logprobabilitiesPositive = torch.log(1/(1+torch.exp(-dhWeights)))
   logprobabilitiesNegative = torch.log(1/(1+torch.exp(dhWeights)))
   ...
   for line in sentence:
      ...
      probability = probabilitiesPositive[stoi_deps[key]]
      dhSampled = (random() < probability.data.numpy())
      if dhSampled:
         line["ordering_decision_log_probability"] = logprobabilitiesPositive[stoi_deps[key]]
      else:
         line["ordering_decision_log_probability"] = logprobabilitiesNegative[stoi_deps[key]]

      direction = "DH" if dhSampled else "HD"
      if printThings: 
         print "\t".join(map(str,["ORD", line["index"], (line["word"]+"           ")[:10], (".".join(list(key)) + "         ")[:22], line["head"], dhSampled, direction, (str(float(probability))+"      ")[:8], str(1/(1+exp(-dhLogits[key])))[:8], (str(round(float(distanceWeights[stoi_deps[key]].data.numpy()), 4))+"    ") , str(originalDistanceWeights[key])[:8]    ]  ))
...
totalVocabCount = sum(y for x, y in vocab.iteritems())
surprisalBaseline = { x : -log(y/float(totalVocabCount)) for x, y in vocab.iteritems()}

while True:
  corpus = list(CorpusIterator(args.language).iterator(rejectShortSentences = False))
  shuffle(corpus)
  refreshInterval = int(0.05* len(corpus))
  refreshInterval = min(1000, max(100, refreshInterval))
  for sentenceID, sentence in enumerate(corpus):
       if counter > 200000:
       ...
       a = time.time()
       if sentenceID % refreshInterval == 0:
          dhWeightsList = [float(x) for x in dhWeights]
          distanceWeightsList = [float(x) for x in distanceWeights]

          print("Start linearizing")
          corpusLinearized = [orderSentenceFast(x, None, None) for x in corpus[:sentenceID] + corpus[sentenceID+refreshInterval:]]
          unigramTable = defaultdict(int)
          bigramTable = {}
          for sent in corpusLinearized:
             sent = ["SOS"] + sent + ["EOS"]
             for i in range(1, len(sent)):
                unigramTable["_TOTAL_"] += 1
                unigramTable[sent[i]] += 1
                if sent[i-1] not in bigramTable:
                    bigramTable[sent[i-1]] = defaultdict(int)
                bigramTable[sent[i-1]]["_TOTAL_"] += 1
                bigramTable[sent[i-1]][sent[i]] += 1
    #      print(unigramTable)
          wordsNotInUnigramTable = [x for x in itos if x not in unigramTable]
          unigramTable["_TOTAL_"] += 10
          for x in wordsNotInUnigramTable:
               unigramTable[x] = len(wordsNotInUnigramTable) / 10.0
          print("Finish linearizing", time.time()-a)
       current = [sentence]
       assert len(current)==1
       batchOrdered, overallLogprobSum = orderSentence(current[0], dhLogits, printHere)
       words = ["SOS"] + [x["word"] for x in batchOrdered] + ["EOS"]
       surprisalMinusBaseline = 0
       surprisals = []
       for i in range(1, len(words)):
         unigramCount = unigramTable[words[i]]
         unigramProb = unigramCount / float(unigramTable["_TOTAL_"])
         preceding = words[i-1]
         word = words[i]
         if preceding in bigramTable:
            probability = (max(bigramTable[preceding].get(word, 0)-1.0, 0.0) + 1.0*(len(bigramTable[preceding])-1) * unigramProb) / float(bigramTable[preceding]["_TOTAL_"])
         else:
            probability = unigramProb
         assert probability >= 0, probability
         assert probability <= 1, probability
         surprisals.append(-log(probability))
         surprisalMinusBaseline += (-log(probability)) - surprisalBaseline[word]
       for i in range(1, len(words)):
          surprisalBaseline[words[i]] = 0.9 * surprisalBaseline[words[i]] + (1-0.9) * surprisals[i-1]
...
#       if maxLength <= 2:
           ...
           for i in range(1,len(input_words)): 
              for j in range(1):
                 if input_words[i][j] != 0:
                    ...
                    depLengthMinusBaselines = depLength - float(baseline_predictions[i][j]) - float(baseline_predictions[realHead][j])
                    dlmLoss += depLengthMinusBaselines
                    lossWords += depLength

           policyGradientLoss = overallLogprobSum * (args.surprisalWeight * surprisalMinusBaseline + (1-args.surprisalWeight) * dlmLoss)
 ...
 #      for x in parameters():
#         print(x.grad.abs().max(), x.data.size())
       ...
       if counter % 10000 == 0:
          TARGET_DIR = "/u/scr/mhahn/deps/DLM_MEMORY_OPTIMIZED/locality_optimized_dlm_surp_weighted/"
          print "Saving"
          with open(TARGET_DIR+"/manual_output_funchead_fine_depl_surp_weighted/"+args.language+"_"+__file__+"_model_"+str(myID)+".tsv", "w") as outFile:
