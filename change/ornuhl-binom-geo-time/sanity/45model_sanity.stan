...
//  print(K1[4, 47])  ;
  ...
  targetPrior += student_t_lpdf(sigma_B | 3, 0, 2.5);
  targetPrior += student_t_lpdf(sigma_Sigma | 3, 0, 2.5);
  targetPrior += normal_lpdf(alpha[1] | 0, 1);
  targetPrior += lkj_corr_cholesky_lpdf(Lrescor_Sigma | 1);


  targetPrior += normal_lpdf(kernel_mu1_rho_time | 0, 1); 
  targetPrior += normal_lpdf(kernel_mu2_rho_time | 0, 1); 

  targetPrior += normal_lpdf(kernel_mu1_alpha | 0, 1); 
  targetPrior += normal_lpdf(kernel_mu1_rho   | 0, 1);  
  targetPrior += normal_lpdf(kernel_mu1_sigma | 0, 1); 
  targetPrior += normal_lpdf(kernel_mu2_alpha | 0, 1);  
  targetPrior += normal_lpdf(kernel_mu2_rho   | 0, 1); 
  targetPrior += normal_lpdf(kernel_mu2_sigma | 0, 1);  

  targetPrior += multi_normal_lpdf(mu1 | zero_mean, K1);
  targetPrior += multi_normal_lpdf(mu2 | zero_mean, K2);
  ...
  for (n in 2:TotalN) {
     ...
     if (ParentIndex[n] == 1) {
        targetLikelihood += multi_normal_lpdf(own_overall | target_mean_here, Omega);
     } else {
...
//         }
        ...
        targetLikelihood += multi_normal_lpdf(own_overall | target_mean_here + exp1 * (reference_overall - target_mean_here), covariance_diagnostic);
     }
     if(!IsHidden[n]) {
