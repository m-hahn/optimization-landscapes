...
parameters {
  ...
  vector<lower=0.02, upper=2>[2] sigma_Lambda;
  vector<lower=0.02, upper=2>[2] sigma_B;
  real<lower=0, upper=0.98> corr_B;
}
...
model {
  ...
  matrix[2, 2] Lrescor_B = [[1, corr_B], [corr_B, 1]];
  matrix[2, 2] Lambda = diag_pre_multiply(sigma_Lambda, Lrescor_Lambda);
