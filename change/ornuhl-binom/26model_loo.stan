...
generated quantities {
  ...
  int n = leaveOut;
     ...
     own_overall = [own_logit, own_trait]';
     reference_overall = [reference_logit, reference_trait]';
     target_mean_here = alpha;
     if (ParentIndex[n] == 1) {
        likelihoodHere += multi_normal_lpdf(own_overall | target_mean_here, Omega);
     } else {
        matrix[2, 2] exp1 = matrix_exp(-B * ParentDistance[n]);
        matrix[2,2] covariance_diagnostic = Omega - exp1 * Omega * exp1';
        likelihoodHere += multi_normal_lpdf(own_overall | target_mean_here + exp1 * (reference_overall - target_mean_here), covariance_diagnostic);
     }
     if(!IsHidden[n]) {
        int success = TrialsSuccess[Total2Observed[n]];
        int total = TrialsTotal[Total2Observed[n]];
