...
with open("../../case_marking_revised.tsv", "r") as inFile:
...
print((la["Sigma_Case"] > 0).mean(axis=0))
print((la["Sigma_NoCase"] > 0).mean(axis=0))
...
with open(f"fits/CORR_Sigma_Case_{__file__}.txt", "w") as outFile:
  for x in la["Sigma_Case"][:,0,1] / np.sqrt(la["Sigma_Case"][:,0,0] * la["Sigma_Case"][:,1,1]):
      print(float(x), file=outFile)
with open(f"fits/CORR_Sigma_NoCase_{__file__}.txt", "w") as outFile:
  for x in la["Sigma_NoCase"][:,0,1] / np.sqrt(la["Sigma_NoCase"][:,0,0] * la["Sigma_NoCase"][:,1,1]):
      print(float(x), file=outFile)
