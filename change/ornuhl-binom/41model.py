...
observedLanguages = [x for x in list(observedLangs) if x in valueByLanguage and caseMarking(x)]
 ...
 #     print([l for l in listOfAllAncestors[l1] if l in listOfAllAncestors[l2]])
  #    print([int(dates.get(l, 2000)) for l in listOfAllAncestors[l1] if l in listOfAllAncestors[l2]])
      ...
      if len(commonAncestorsDates) > 0:
         ...
         commonTime = -50000
      separateTime = (int(dates.get(l1, 2000)) - commonTime) + (int(dates.get(l2, 2000)) - commonTime)
      assert separateTime > 0 or l1 == l2, (dates.get(l1, 2000), dates.get(l2, 2000), commonTime, l1, l2)
      if commonTime > -50000:
         print(l1, l2, separateTime)
      covarianceMatrix[i][j] = (separateTime)/1000
      covarianceMatrix[j][i] = (separateTime)/1000
