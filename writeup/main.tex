...
Human languages show both tremendous variation and striking similarities in their syntactic structure \citep{greenberg-universals-1963, baker2001atoms, croft2003typology}. A large body of research suggests that cross-linguistic variation and similarities are driven by functional pressures of efficient communication under limited cognitive resources \citep{haspelmath2008parametric, jaeger2011on, kemp2018semantic, gibson2019how}. Here we present evidence that the evolution of syntactic structure reflects an additional functional pressure through joint optimization of the grammatical structure of a language and the way that language is used.
...
We draw on the Universal Dependencies data, which includes text data annotated with syntactic tree topologies from over 70 languages from 29 language families (CITE, see Materials and Methods for details).
...
Phylogenetic analyses have previously been applied to studying the historical evolution of languages~\citep{gray2009language,sagart2019dated,greenhill2009austronesian}, including the evolution of word order patterns \citep{dunn-evolved-2011, maurits2014tracing}.
...
We also found such an association between order and coexpression within individual languages: Languages with word order flexibility tend to order subjects differentially depending on the presence or absence of an object, in a way consistent with DLM (see SI Section X).

%Ergative languages that are SVO and VS, e.g. in Mesoamerica %(https://wals.info/chapter/82)\
%There are also a few OVS/SV languages
...
It has been argued sometimes that there is an inherent directionality in the evolution of basic word order, and that SOV is the default or original order in the history of language.
...
In existing efficiency-based models, differences among languages have been interpreted as reflecting different optima or points along a Pareto frontier \citep{kemp2018semantic, zaslavsky2018efficient}.
...
We obtained the topology of phylogenetic trees from Glottolog~\citep{nordhoff2011glottolog}, inserted documented historical languages as inner nodes, and assigned dates for the other inner nodes based on the literature (see SI Section X for details).
...
\paragraph{Data Avilability} All code and results are freely available at \url{https://github.com/m-hahn/optimization-landscapes/}.
...
We implemented the models in Stan and obtained posterior samples using the No-U-Turn sampler as implenented in Stan~\citep{carpenter2017stan}.
...
However, linguistic evolution can inmclude borrowing between geographically neighboring languages (CITE).
%\cite{kalyan2019problems}
Here, we show that our conclusions continue to hold when explicitly modeling convergence in linguistic areas (CITE), geographic regions in which languages tend to show convergent evolution due to borrowing.
...
The Laplace kernel is positive-definite with the great-circle distance $d(\cdot, \cdot)$ \citep{feragen2015geodesic}, whereas the commonly used RBF kernel is not positive-definite.
...
We extracted locations of languages from the World Atlas of Linguistic Structures (CITE).
