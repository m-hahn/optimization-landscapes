...
\textcolor{red}{TODO at some point more disucssion on the dataset? selection of languages is restricted by data availability. while we verified that results hold even when excluding the highly prominent Indo-European family, selection is still biased (e.g. only one langusge rewpresenting the Americas.) as data from more languages is  becoming avilable, can keep verifying predictions on those in the future. }
