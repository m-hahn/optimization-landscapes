...
%=======
%This line of work typically assumes that communicative need as a constant that constrains what language structures are efficient for those communicative needs. % (CITE e.g. some of the efficiency work in semantic typology).
Existing work has also suggested that communicative need, or how frequent a linguistic element is used by its speakers, differs across languages, and that it is responsible for some of the differences observed among the structures of different languages \cite{perkins1992deixis, lupyan2010language, gibson2017color}. % (perhaps also Sapir, Piraha, ...).
...
%Here, we draw on large-scale data and phylogenetic modeling to argue that usage and grammar are optimized jointly to maintain efficiency in language change:Languages tend to be structured efficiently for the utterances that speakers produce, and, conversely, speakers structure information in ways that are efficient given the grammar of their language.
...
Without controlling for dependencies between related languages, optimized and attested subject-object position congruence are strongly correlated ($R=0.47$, $95\%$ CI $[0.28, 0.64]$, $p<10^{-4}$). % analysis/output/correlation.txt
