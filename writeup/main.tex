...
Our contribution is to provide large-scale evidence for coadaptation between grammar and usage, in the domain of word order.
...
Our proposal of coadaptation suggests that languages evolve to have the basic word orders that are most efficient for DLM, given the utterances speakers of a language typically communicate.
...
We quantify the position of an individual language on the continuum between SVO and SOV/VSO using a quantitative corpus-based metric which we refer to as subject-object position congruence.
...
\subsection*{Data}
We drew on the Universal Dependencies 2.6 treebanks, including every language for which data with at least 500 sentences was available, while excluding code-switched text, text produced specifically by nonnative speakers, and sign languages.
