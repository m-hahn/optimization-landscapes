...
We first consider a simple transitive sentence such as `dog bites a human' (Figure~\ref{fig:sent-dep}). \mhahn{TODO what to do about the English article? -- it distracts a bit by making dependencies longer. Maybe make it `dogs bite people', so it's more parallel to the Japanese?}
...
We contrast two hypotheses: Basic word order reflects coadaptation between grammar and usage, and basic word order reflects different optima, or is not driven by optimization.
In the first case, we expect that languages tend to have the basic word order that is most efficient given their usage distributions.
In th second case
...
Third, we use a quantitative corpus-based metric which we call \key{Subject-Object Symmetry}.
This is the chance that two randomly selected instances of S and O from the corpus -- not necessarily from the same sentence -- are on the same side of their respective verbs.

%We propose two quantitative metrics measuring where a language falls on this spectrum.
%First, we count how often S and O appear on the same side of a verb that realizes both S and O arguments (\key{Same-Side Count}).\mhahn{Are there better names for these measures?}
...
This result agrees with the predictions of the \textsc{Coadaptation} hypothesis: Languages tend to have the basic word order that is most efficient given their tree structure distributions.
...
In a linear mixed-effects models, with by-family intercept and slope, subject-object symmetry was predictive of this fraction ($\beta=-0.11$, $SE=0.05$, $95\%$ CrI $[-0.21, -0.01]$).\mhahn{TODO the other measure}
