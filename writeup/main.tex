...
%\title{Grammar and Communicative Need Interact in the Evolution of Word Order across Languages {\color{blue}Change?}}
\title{Grammar and Frequency Co-adapt in the Evolution of Word Order across Languages}
...
Extensive work has sought to explain word order variation across languages.
...
We propose a theory of coadaptation between grammar and frequency that extends Zipf's law of abbreviation to syntax: frequent syntactic usages tend to have low dependency lengths, both within and across languages.
We show in 74 languages from 29 families that they exhibit the basic word order that is most efficient given the sentences typically produced by their speakers, under the joint consideration of usage frequency and grammar for Dependency Length Minimization.
	Drawing on phylogenetic modelling and historical text corpora from seven language families, we show that historical word order change is accompanied by change in the frequency distribution of propositions that speakers communicate \textcolor{blue}{TODO let's change 'propositions' to something else}.
We identify relevant characteristics that reflect this joint optimization, particularly the frequency with which subjects and objects are expressed together for the same verb.
Our findings suggest that syntactic structure and usage evolve across languages to support efficient communication with least effort.
\end{abstract}
...
\dropcap{T}he world's languages show considerable variation in grammatical structure \citep{greenberg-universals-1963, baker2001atoms, croft2003typology}.
...
This line of work often takes usage frequency as a constant that constrains what language structures are efficient for those communicative needs. % (CITE e.g. some of the efficiency work in semantic typology).
However, there is also evidence that usage frequency differs between languages, and that this is responsible for some of the differences observed between the structures of different languages \cite{perkins1992deixis, lupyan2010language, gibson2017color}. % (perhaps also Sapir, Piraha, ...).
We thus expect that the evolution of language structure should go hand in hand with changes in the ways its speakers communicate and structure information.
%>>>>>>> c30b2c8b483242e9c8cf900bce79422dc6718eec

Here, we draw on large-scale data and phylogenetic modelling to argue that usage and grammar are optimized jointly to maintain efficiency in language change:
Languages tend to be structured efficiently for the utterances that speakers produce, and, conversely, speakers structure information in ways that are efficient given the grammar of their language.
...
Other orders such as verb-subject-object (VSO, ``bites--dog--human'') are much less common.
Many languages have more than one ordering, though typically with one of the orderings being the most common or least marked.
...
The low frequency of object-initial orderings (e.g., OSV, ``human--dog--bites'') arguably has satisfactory explanations (CITE), but there is no consensus on the frequency distribution of SVO and SOV.
...
We present a new account for the frequency distribution of SOV and SVO order and the evolution of word order across languages:
Coadaptation between grammar and usage frequencies. We refer to this idea as the coadaptation hypothesis.
We propose that neither SVO nor SOV is more efficient in principle.
Rather, languages tend to have the basic word order that is most communicatively efficient given the way its speakers typically structure information.
As languages change, usage frequency and grammar evolve together, such that change in usage frequency goes hand-in-hand with change in basic word order.


The idea that the structure of language is shaped by the way it is used has been discussed in several aspects of language.
...
In this work, we provide large-scale evidence for coadaptation between grammar and usage, in the domain of word order.
...
We show how English (SVO, center) and Japanese (SOV, right) linearize different syntactic structures into strings of words.
...
Syntactic structures in A contain both a subject and an object; those in B only express one of the two.
	We also show usage frequencies computed from large-scale English and Japanese texts (see SI Section TODO).
...
Therefore, structures as those in B are more favorable for DLM in SOV languages than those in A.
...
For instance, the grammar of English specifies subject--verb--object order for (X), the grammar of Japanese specifies subject--object--verb for (X).
...
Figure~\ref{fig:sent-dep} shows how the same syntactic structures are linearized in English (SVO) and Japanese (SOV).
In a simple sentence as in Figure~\ref{fig:sent-dep}A, SVO order results in overall lower dependency length (2 instead of 3).
...
There are also syntactic structures where SOV achieves strictly shorter dependency lengths, such as those in Figure~\ref{fig:sent-dep}C.
...
%In the simplified setting of Figure~\ref{fig:sent-dep}, SVO order achives an average dependency length of 
%TODO introduce the optimization idea here and also in visualization.
%
We propose that languages evolve so that usage frequencies (which syntactic structures are chosen by speakers) and word order (how they are linearized) maintain optimality for DLM.
...
%In SVO order, both dependencies have length $1$, resulting in a total length of $2$.
%In SOV order, the arc between the verb and its subject is longer because it crosses the object, resulting in a total length of $3$.
%%However, there are other syntactic structure configuration where the picture is reversed:
%%In Figure~\ref{fig:sent-dep}B, we consider sentences of the form `I think (that) big dogs bite people'.
%%Here, dependency length is 5 in SVO order, and 4 in SOV order, reversing the advantage  of SVO order seen in basic sentences.
%%Which order is favored by DLM overall?
%This depends on the frequency distribution with which speakers of a language use different syntactic structures:
...
%(TODO here explain the idea of an optimized grammar for DLM and what the optimal basic word order would be)



%The reasoning above focused on SVO and SOV, but this approach can be extended to the less common other possible orderings.
...
%(TODO this has to be folded into the introduction)
...
We then use a model of drift on phylogenetic trees to assess whether languages have evolved historically toward states where usage frequencies and basic word order are aligned.
%>>>>>>> c30b2c8b483242e9c8cf900bce79422dc6718eec
...
If there is coadaptation between basic word order and usage frequencies, we expect that languages should have the basic word order that is most efficient given the distributions of syntactic structures communicated by their speakers.
...
%<<<<<<< HEAD
We estimate both usage frequencies of syntactic structures and the observed subject-object position congruence using datasets of tree structures from 74 languages from 29 language families (\citep{zeman2020universal}, see Methods for details).
...
%=======
%We compare the orderings actually found in a language with the distribution that would be optimal for DLM, given the tree structure distributions for the language.
%We represent such optimal orderings using the  established model of counterfactural order grammar introduced by \cite{gildea-optimizing-2007} (see Materials and Methods).
...
%
%We approximate communicative need using datasets of tree structures from 74 languages from 29 language families (\citep{zeman2020universal}, see Methods for details).
%These datasets contain written text for 73 languages, and transcribed spoken text for six languages (see Methods).
%>>>>>>> c30b2c8b483242e9c8cf900bce79422dc6718eec
...
%We compare the orderings actually found in a language with the optimal orderings. %distribution that would be optimal for DLM, given the tree structure distributions for the language.
%Optimized grammars are computed based entirely on the distribution of unordered tree topologies occurring in the corpora, e.g. how often verbs express one or two arguments (Figure~\ref{fig:sent-dep}), without any reference to the orderings observed in the corpora.
%These optimized grammars reflect what orderings would be most efficient given the usage distribution of tree topologies as reflected in the corpus, independently of the actual orderings that the grammar of the language specifies.
...
In Figure~\ref{fig:study1}A, we visualize actually observed subject-object position congruence against the average subject-object position congruence of the grammars  approximately optimized  for usage distributions over syntactic structures, in the 74 languages.
...
It is incompatible with previous suggestions that SVO is generally more efficient (than SOV) for DLM \citep{ferrer-i-cancho-placement-2017}.
...
If variation in word order reflects coadapation between grammar and usage frequencies, we should expect to see coevolution of basic word order and usage frequency as languages evolve over time.
To test this proposal, we conducted a phylogenetic analysis on the evolution of usage frequency and word order.
...
We used a model of random walks on phylogenetic trees \citep{felsenstein1973maximum,pagel1997inferring, pagel2004bayesian} to model how grammar and usage frequencies evolve over time, in order to test the coadaptation hypothesis over history.
...
\subsection*{Influence of Communicative Need on Word Order}
So far, our results provide evidence for coadaptation between usage and basic word order in the evolution of language.
In what aspects of usage frequencies do languages vary and change that reflect this coadaptation?
...
This shows that languages differ in the rate at which they coexpress subjects and objects, and that this is a factor through which frequency statistics can influence word order.
...
We have investigated the frequency distribution and historical change of word order  across languages and found that usage frequency and basic word order interact intimately in their evolution,  reflecting a process of coadaptation  between usage and grammar.
...
\acknow{We thank Edward Gibson, Vera Gribanova, Boris Harizanov, Dan Jurafsky, Charles Kemp, Terry Regier, and Guillaume Thomas for helpful feedback on the manuscript. This research is supported by a NSERC Discovery Grant RGPIN-2018-05872 and a SSHRC Insight Grant \#435190272 to YX.}
