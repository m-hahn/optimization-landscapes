...
A wide range of existing work has argued that the structure of human language supports efficient communication. In these studies, variation across languages can reflect different optima or points along a Pareto frontier.

However, these theories are not able to explain the fine-grained distribution and evolution of word order.
{\color{red} (add one sentence maybe begin with ``However,'' that explicates the significance of our work and its impact on how efficient communication theory or related work is limited/unsettled in their ability to explain the fine-grained distribution and evolution of word order, as one of the most exemplary domains of cross-linguistic variation in syntactic structure.)} We report evidence for an additional functional pressure in the evolution of word order across languages: Coadaptation between usage and grammar.
...
\cite{ferrer-i-cancho-placement-2017} suggest that SOV and SVO are optimal under different efficiency criteria {\color{red} without ...? or, but ...?}.
{\color{blue}Here we show that the efficiency considerations can make fine-grained predictions about word order in individual languages, which crucially depend on language-specific usage patterns.}
...
We draw on the Universal Dependencies data, which includes text data annotated with syntactic tree topologies from over 70 languages from 29 language families (CITE, see Methods for details).
...
\citet{ferrer-i-cancho-placement-2017} argues that DLM favors SVO in general, and our results show this is not the case (Figure~\ref{fig:study1}) {\color{red} say a bit more -- so what?}.
...
Variation in the grammars of different languages, and change of languages over time, pose an interesting question for existing efficiency-based theories of linguistic structure.
