...
Rather, languages tend to have the basic word order that is most communicatively efficient given the way its speakers typically structure information\textcolor{blue}{, and, conversely, speakers structure information in ways that are most efficient given the basic word order of the language.} As languages change, syntactic usage and grammar evolve together such that change in frequency of syntactic usages should go hand-in-hand with change in basic word order. \textcolor{green}{Our proposed theory of coadaptation makes no claim about the causal direction between grammar and usage, and it only claims that grammar and usage frequencies should evolve in tandem. While, in a given language, a change might first appear in one aspect (e.g., language use), the preservation of efficiency can lead to compensating changes in another aspect (e.g., grammar). Consequently, grammar and usage constrain each other in the process of evolution.} We provide large-scale evidence for coadaptation in word order both cross-linguistically and over the course of language evolution.
...
We therefore reran the phylogenetic analysis excluding the Indo-European phylum.
...
\subsection*{Influence of Usage on Word Order} \mhahn{maybe rename this?}
...
\textcolor{green}{Our results do not speak to the causal direction of changes in individual languages. Our currently available data are consistent with causal influence in either direction with the possibility of a third hidden causal factor (see SI Section X for analyses).
