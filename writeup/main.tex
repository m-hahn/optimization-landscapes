...
\begin{figure}
    \centering
    \includegraphics[width=0.3\textwidth]{analysis_spoken/spoken.pdf}
...
We model the change of $\xi_{L,t}$ over time using an Ornstein-Uhlenbeck process (see Materials and Methods for details).
This model allows us to determine the dynamics of how $X_{L,t}, Y_{L,t}$ evolve:
Under the coadaptation hypothesis, changes in both variables should be positively correlated.
Under the null hypothesis, no such correlation is expected.



This model draws statistical evidence both from synchronic data and from historical corpora.
In several cases, our dataset includes data from different stages of the same language (such as Old English and Modern English).
Such datasets provide direct evidence of diachronic development.
Using phylogenetic information, the model can also draw strength from purely synchronic data.
Data from related languages may permit inferences about their (not directly documented) common ancestor, and thus about trajectories of change.
...
In the resulting model, change in the two directions was positively correlated (posterior probability of negative or no correlation XXX).
We compared the model with a simpler version in which usage and grammar evolve independently, finding that the full model provided better model fit (Bayes Factor TODO).
...
We adopt the model of Brownian motion on phylogenetic trees, with an added drift term.
...
We model the instantaneous change of the state $\xi_t$ a language at a given time $t$ using the following stochastic differential equation, describing an Ornstein-Uhlenbeck process:
\begin{equation*}
    d\xi_t = \Gamma \cdot (\xi_t-\mu) dt + \Lambda dB_t
\end{equation*}
where $\mu \in \mathbb{R}^2$,  $\Gamma, \Lambda \in \mathbb{R}^{2\times 2}$ are non-degenerate matrices, and $\Gamma$ is diagonal with positive entries (see SI Section X for non-diagonal $\Gamma$).
The first term encodes drift towards an axis $\mu$, and describes which region of parameter space languages tend to move towards.

The second term encodes the dynamics of change.
$\Lambda_{1,2}$ encodes the extent to which drift of $X_t$ and $Y_t$ is correlated:
$\Lambda_{1,2} > 0$ means that languages drift in such a way that $X_L, Y_L$ are positively correlated (coadaptation); $\Lambda_{1,2} = 0$ means that movement in both directions is not correlated.
$\Gamma$ and $\mu$ encode 


See SI Section X for implementation details, for comparison with a simple Brownian motion model, and for comparison with non-diagonal $\Gamma$.
