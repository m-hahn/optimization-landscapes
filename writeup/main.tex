...
Drawing on phylogenetic modelling and historical text corpora from seven language families, we show that historical word order change is accompanied by change in the distribution of propositions that speakers communicate.
...
Our findings highlight that functional optimization in language structure and in language usage go hand in hand in shaping the evolution of syntactic structure across languages.{\color{blue}Change?}
\end{abstract}
...
%<<<<<<< HEAD
%This line of work often takes communicative need as a constant that constrains what language structures are efficient for those communicative needs (CITE e.g. some of the efficiency work in semantic typology).
%However, there is also evidence that communicative need differs between languages, and that this is responsible for some of the differences observed between the structures of different languages \cite{perkins1992deixis, lupyan2010language, gibson2017color} (perhaps also Sapir, Piraha, ...).
%If this is true, then changes in the communicative needs of its speakers should go hand in hand with the evolution of language structure.
%=======
This line of work often takes communicative need as a constant that constrains what language structures are efficient for those communicative needs. % (CITE e.g. some of the efficiency work in semantic typology).
However, there is also evidence that communicative need differs between languages, and that this is responsible for some of the differences observed between the structures of different languages \cite{perkins1992deixis, lupyan2010language, gibson2017color}. % (perhaps also Sapir, Piraha, ...).
...
Coadaptation between grammar and communicative preferences. We refer to this idea as the coadaptation hypothesis.
...
%
We propose that languages evolve so that communicative preferences (which syntactic structures are chosen by speakers) and word order (how they are linearized) maintain optimality for DLM.
...
We then use a model of drift on phylogenetic trees to assess whether languages have evolved historically toward states where communicative need and basic word order are aligned.
%>>>>>>> c30b2c8b483242e9c8cf900bce79422dc6718eec
...
If there is coadaptation between basic word order and communicative need, we expect that languages should have the basic word order that is most efficient given the distributions of syntactic structures communicated by their speakers.
...
%<<<<<<< HEAD
We estimate both communicative need and the observed subject-object position congruence using datasets of tree structures from 74 languages from 29 language families (\citep{zeman2020universal}, see Methods for details).
...
We have investigated the frequency distribution and historical change of word order  across languages and found that communicative need and basic word order interact intimately in their evolution,  reflecting a process of coadaptation  between communicative need and grammar.
