...
Human languages show both tremendous variation and striking similarities in their syntactic structure (cite Greenberg, see my comments in pdf). A large body of research suggests that cross-linguistic variation and similarities are driven by functional pressures of efficient communication under limited cognitive resources (Haspelmath, 2008; cite at least two more review papers, see my comments in pdf, Cite more here, including the recent TICS paper by Gibson et al., as well as the ARL review paper by Kemp, Xu, and Regier.). Here we present evidence that the evolution of syntactic structure reflects an additional functional pressure through joint optimization of the grammatical structure of a language and the way that language is used.
...
Typologists have long classified languages according to their basic word order, or the order in which they typically order verbs, subjects, and objects.
About 40\% of the world's languages are classified as following subject-verb-object order ({SVO}, as in English, ``a dog bites a human''), and 40 \% are classified as following subject-object-verb order ({SOV}, ``dog-human-bites'') (CITE).
...
This ordering is considered the  basic word order of a language in the typological literature (CITE).
