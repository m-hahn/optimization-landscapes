...
\begin{figure}
    ...
    \caption{Study 1: Subject-object symmetry and real and optimized orderings. x-axis real order distributions, y-axis model prediction.}
    \label{fig:study1}
\end{figure}
...
Data from related languages may permit inferences about their (undocumented) common ancestor, and thus about possible trajectories of historical change.


We used a model of random walks on phylogenetic trees to model how grammar and usage of a language evolve over time, and to test the Coadaptation Hypothesis.
...
We model the change of $\xi_{L,t}$ over time using a random walk given by an Ornstein-Uhlenbeck process (see Materials and Methods for details).
This process is parameterized by, first, rates of change in the two dimensions and, second, the correlation between the changes in both dimensions.
...
\begin{figure}
    ...
    \caption{Historical Changes: We show the trajectory of change in seven languages or language families with documented ancestors. For each point in the trajectories The blue background distribution indicates the long-term stationary distribution of languages }
...
We adopt the model of random walks on phylogenetic trees (CITE).
We model the state of the grammar as $X_L$, the observed subject-object symmetry.
...
A common choice in phylogenetic modeling of coevolving traits is correlated Brownian motion (CITE).
...
This leads to an Ornstein-Uhlenbeck process (CITE), described by the following stochastic differential equation for the instantaneous change of the state $\xi_{L,t}$ of a language $L$ at a given time $t$:
\begin{equation*}
    d\xi_{L,t} = \Gamma \cdot (\xi_{L,t}-\mu) dt + \Lambda dB_t
\end{equation*}
where $\mu \in \mathbb{R}^2$,  $\Gamma, \Lambda \in \mathbb{R}^{2\times 2}$ are non-degenerate matrices, $\Gamma$ is diagonal with positive entries (see SI Section X for results with non-diagonal $\Gamma$), and $B_t$ is Brownian motion in two dimensions.

The first term, $\Gamma \cdot (\xi_{L,t}-\mu) dt$, encodes deterministic drift, and describes which region $\mu \in \mathbb{R}^2$ of parameter space languages tend to concentrate around in the long term.
...
The second term, $\Lambda dB_t$ describes the dynamics of stochastic change.
The diagonal entries of $\Lambda$ encode rates of change in the two dimensions; the off-diagonal encodes correlation: Positive entries in the off-diagonal mean that languages drift in such a way that $X_L, Y_L$ are positively correlated; zero off-diagonal entries indicate independent change in both dimensions.
...
We conducted Bayesian inference using Hamiltonian Monte Carlo in Stan (CITE) and computed Bayes factors using Stepping Stone Sampling (CITE).
...
Slavic       & 700AD & \citep{gray2003language} \\
...
Iranian & 500 BC & \citep{gray2003language} \\ % Kurmanji, Persian. Parpola 1999 suggests 1900 BC.
...
North-Germanic & 600 AD & Split of Old Norse into regional variants. \citep{sandøy2017202} \\% (e.g. assimilation of nasals to following stop in Western Norse in the 7th century. Bandle 2005, Ch. XVII par. 202, The typological ...I Phonology Old East Nordic, p. 1856, 1859. \\
