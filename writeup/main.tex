...
%It has also been argued that the sound systems of languages adapt to climate and diet of the speech community \citep{everett2015climate,blasi2019human}.
Here we extend these ideas to investigate the possibility that the interaction of syntactic use and grammar might shape the variation and evolution of word order across languages.
...
Rather, languages tend to have the basic word order that is most communicatively efficient given the way its speakers typically structure information\textcolor{blue}{, and, conversely, speakers structure information in ways that are most efficient given the basic word order of the language.} As languages change, syntactic usage and grammar evolve together such that change in frequency of syntactic usages should go hand-in-hand with change in basic word order. \textcolor{blue}{Our proposed theory of coadaptation makes no claim about the causal direction between grammar and usage, and it only claims that grammar and usage frequencies should evolve in tandem. While, in a given language, a change might first appear in one aspect (e.g., language use), the preservation of efficiency can lead to compensating changes in another aspect (e.g., grammar). Consequently, grammar and usage constrain each other in the process of evolution.} We provide large-scale evidence for coadaptation in word order both cross-linguistically and over the course of language evolution.
...
Our proposal of coadaptation suggests that languages have the basic word orders that are most efficient for DLM, given the ways speakers of a language typically structure information\textcolor{blue}{, while speakers structure information in ways efficient for DLM.}
...
In order to efficiently construct grammars that approximately minimize average dependency length, we use the gradient descent algorithm from \cite{hahn2020universals}.
...
\begin{figure*}[h!]
    \centering
	\begin{tabular}{cc}
		(A) All Languages & (B) Indo-European Excluded \\
		\includegraphics[width=.45\textwidth]{../analysis/figures/fracion-optimized_DLM_2.6_format.pdf} &
                \includegraphics[width=0.45\textwidth]{../analysis/figures/fracion-optimized_DLM_2.6_format_NoIE.pdf}
	\end{tabular}
        
	\caption{\mhahn{TODO I'll still make sure to align the colors, font size, etc.} Evaluation of the coadaptation hypothesis across languages. subject-object position congruence in attested (x-axis) and approximately optimized (y-axis) orderings. Languages are colored by language families as coded by \cite{zeman2020universal}. Small lines connect labels to the position of the language, where needed to avoid overlapping labels.} % b) Comparison of subject-object position congruence between written (blue) and spoken (red) corpora in six languages where spoken corpora are available. Note that, for one language (Naija), all available data is in spoken form.}
...
%\begin{figure*}[h!]
%    \centering
%    \includegraphics[width=.5\textwidth]{../analysis/figures/fracion-optimized_DLM_2.6_format.pdf}
%    
%
%                    \includegraphics[width=0.4\textwidth]{../analysis/figures-scratch/within-IE.pdf}
%                \includegraphics[width=0.4\textwidth]{../analysis/figures/fracion-optimized_DLM_2.6_format_NoIE.pdf}
%        
...
%\begin{figure*}
%    \centering
%            \includegraphics[width=0.8\textwidth]{../analysis/figures-scratch/per-phylum-posteriors-short.pdf}
...
\textcolor{blue}{A possible concern is that some language families are represented more than others; in particular, 46 of the languages belong to the Indo-European phylum.
The correlation between optimized and attested subject-object position congruence was also significant when excluding the Indo-European languages ($R=0.68$, $95\%$ CI $[0.40, 0.84]$, $p=0.00013$; Spearman's $\rho=0.66$, $p=0.00022$).
To account for the statistical dependencies between related languages more rigorously, we grouped the 29 language families into 14 phyla describing maximal units that are not genetically related to each other (see Materials and Methods), and performed a regression analysis where we entered phyla as random effects with random slopes and intercepts.}
\textcolor{blue}{Attested subject-object position congruence was predictive of optimized subject-object position congruence ($\beta = 0.40$, $SE=0.11$, $95\%$ credible interval $[0.18, 0.64]$, $P(\beta\leq 0) = 0.00075$, Bayesian $R^2 = 0.30$; see Materials and Methods for details)}. % analysis/output/landscapes_2.6.R_brms_phyla_linear.txt ,
...
%\mhahn{if we really remove the left figure, then we have to rephrase here}
...
%According to the model, changes in grammar and syntactic usage frequency are positively correlated and, 
...
Figure~\ref{fig:historical} also visualizes the evolution of languages where multiple historical stages are documented. %(right)
...
%    \includegraphics[width=0.32\textwidth]{../change/visualize/stationary.pdf}
 %   \includegraphics[width=0.65\textwidth]{../analysis/figures/historical_2.6_times_stationary_layout.pdf}
    \includegraphics[width=0.95\textwidth]{../analysis/figures/historical_2.6_times_stationary_layout_new2.pdf}
    

	\caption{
	Result of the phylogenetic drift analysis.
...
%Right:
	The evolutionary trajectories of seven languages or language families with documented ancestors. For each point in the trajectories, we indicate an approximate date. The blue background distribution indicates the long-term stationary distribution of all languages combined, including languages without historical data, as inferred by the phylogenetic analysis. The long-term stationary distribution indicates that languages evolve so that grammar (x-axis) and usage frequencies (y-axis) are aligned.}
    \label{fig:historical}\label{fig:drift-model}
\end{figure*}


\textcolor{blue}{%One possible concern in our analysis is that the available corpus data may over-represent certain language families. In particular, 46 of the languages belong to the Indo-European phylum.
We also reran the phylogenetic analysis excluding the Indo-European phylum.
...
These results show that the fit of the phylogenetic model is representative of the entire dataset, not just the well-represented Indo-European phylum.
} 
...
\textcolor{blue}{Specifically, we labeled each languages in our dataset, including the inner nodes of the phylogenetic tree, for four conventional discrete word order categories (``SVO'', ``SOV'', and rarer categories ``VSO'', ``No dominant order''). Annotations were based on typological databases for documented languages \cite{wals-81}, and on reconstructions \cite{gell-mann-origin-2011} for undocumented ancestors (see SI  Appendix, Section S6 for details).}
...
\subsection*{\textcolor{blue}{Relation between Usage and Word Order}} So far, our results provide evidence for coadaptation between usage and basic word order in the evolution of languages.
...
\textcolor{blue}{Our results do not speak to the causal direction of changes in individual languages. Our currently available data are consistent with causal influence in either direction with the possibility of a third hidden causal factor (see SI Section S13 for analyses).
In the future, the availability of historical data with higher temporal resolution might make it possible to explore the causal direction of change in individual languages.}
...
\textcolor{blue}{
A limitation of our results is that corpus data with syntactic annotation is available primarily for European and Asian languages.
Alternative approaches to estimating usage distributions could leverage either manual annotation of text for relevant quantities such as subject-object coexpression, or automatically projecting syntactic annotation to other languages on multilingual text, such as Bible translations.
However, those approaches would either not reflect usage distributions in sufficient detail to identify optimized orderings, or risk reflecting non-idiomatic properties of translated text.
%
%It might be possible to use other data sources to test our hypothesis, for instance by manual annotation of subject-object position congruence and the rate of coexpression of subjects and objects on text data.
%However, scaling such an approach to a number of languages giving strong statistical power, and the approach would still not represent usage distributions in sufficient detail to identify optimized orderings.
%Approaches relying on automated projection of syntactic annotation to other languages result in lower quality data and are applicable primarily for translated text, not necessarily representative of idiopmatic usage.
%Thus, the Universal Dependencies corpora remain the best data source for testing the coadaptation hypothesis.
}

\textcolor{blue}{Our study focused on basic word order, but the theory of coadaptation makes predictions more generally for crosslinguistic variation in word order. Testing its predictions in other domains is an interesting problem for future research.}
...
Optimized grammars are created using the gradient descent method described by \citep{hahn2020universals} \textcolor{blue}{See SI Appendix, Section S14 for more details and examples.}.
...
\subsection*{Bayesian Regression Analyses}
\textcolor{blue}{
...
Following \citep{burkner2018advanced}, we assumed a uniform prior for the slope, a weakly informative prior (Student's $t$ with $\nu=3$ degrees of freedom, location 0, and scale $\sigma=2.5$) for the fixed the variance components, and an LKJ(1) prior \citep{lewandowski2009generating} for the covariance matrix of random effects. % (i.e., the PDF $p$ is $\frac{1}{\sigma} p_3(x/\sigma)$, where $p_3$ is the PDF of the $t$-distribution with $\nu=3$)
See SI Section S13 for results with more strongly regularizing priors, and for corresponding frequentist analyses.
We computed Bayesian $R^2$ values following \cite{gelman2019r}.
We report $P(\beta \leq 0)$, the posterior probability that the coefficient is not positive, to quantify the fraction of the posterior supporting the hypothesis of a positive coefficient.
The phyla in the random effects structure correspond to the maximal subgroups in the phylogenetic trees described in `Data'.
}
...
We model the state of the grammar of a language $L$ as a tuple $\xi_L \in [0,1]^2$ consisting of $X_L$, the observed subject-object position congruence, and $Y_L$, the average subject-object position congruence of optimized grammars.
