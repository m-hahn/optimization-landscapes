...
When predicting coexpression of subjects and objects from attested subject-object position congruence, the slope was TODO (SE TODO, t TODO), with TODO.
...
Results show how the mixed-effects model combines statistical evidence across the phyla.
...
This model describes how stochastic change -- expressed by $\Lambda \operatorname{d}B_t$ -- tends to be correlated in usage and grammar, and does not encode a direction of causal effect between usage and grammar.
We compared this with a model that explicitly builds in a causal direction, by assuming that one dimenion (e.g., $\xi_1$) changes independently, whereas the other (e.g., $\xi_2$) is drawn to the first one:
\begin{align*}
...
It expresses an asymmetric causal relation, where changes in $\xi_2$ are caused by $\xi_1$.
The strength of influence of $\xi_1$ on $\xi_2$ is expressed by the term $\Gamma_{12}$.
...
We compared marginal likelihood across the three models in Table~\ref{tab:causal}.
On our dataset, all three models provide almost the same marginal likelihood, showing that currently available data does not make it possible to decide whether there is a single asymmetric causal direction between usage and grammar across languages.


\begin{table}
	\begin{center}
\begin{tabular}{l|llll}
	Model & Marginal Likelihood \\ \hline
	Coevolution (as described in Main Paper) & -32 \\
%	No Coevolution (lesioned, as describined in Main Paper) & -38 \\
	Grammar shapes Usage & -32 \\
	Usage shapes Grammar & -32 \\
\end{tabular}
	\end{center}
