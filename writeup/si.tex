...
We conducted standard Bayesian linear mixed-effects regressions (CITE) where the response $y_i$ belonging to language $i$ is given by
\begin{equation}
...
We repeated the analysis with several more strongly regularizing priors on both the variance components and the slope $\beta$.
...
Figure~\ref{fig:per-phylum-plot} shows results across the 14 phyla.
...
\begin{equation}
\begin{matrix} \sigma_1 & 0 \\ 0 & \sigma_2\end{matrix}
\end{equation}
...
Behavior of the three models is similar, and one needs high temporal resolution to choose between models.

On our dataset, all three models provide almost the same marginal likelihood.
