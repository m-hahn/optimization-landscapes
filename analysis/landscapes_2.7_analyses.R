...
u = read.table("landscapes_2.6_new.R.tsv") %>% filter(!(Language %in% c("Persian_2.6", "Turkish_2.6", "Faroese_2.6", "Icelandic_2.6")))
v = read.table("landscapes_2.7_new.R.tsv")
u = rbind(u,v)
real6 = read.table("landscapes_2.6_new_real.R.tsv") %>% filter(!(Language %in% c("Persian_2.6", "Turkish_2.6", "Faroese_2.6", "Icelandic_2.6")))
real7 = read.table("landscapes_2.7_new_real.R.tsv")
real = rbind(real6, real7)
data6 = read.table("landscapes_2.6_new_data.R.tsv") %>% filter(!(Language %in% c("Persian_2.6", "Turkish_2.6", "Faroese_2.6", "Icelandic_2.6")))
data7 = read.table("landscapes_2.7_new_data.R.tsv")
data = rbind(data6, data7)

library(brms)
sink("output/landscapes_2.7.R_avgs.txt")
...
library(lme4)
sink("output/landscapes_2.7.R.txt")
...
capture.output(summary(model), file="output/landscapes_2.7.R_brms.txt")
samp = posterior_samples(model)
capture.output(mean(samp$b_OSSameSide_Real_Prob_Log > 0), file="output/landscapes_2.7.R_brms.txt", append=TRUE)
