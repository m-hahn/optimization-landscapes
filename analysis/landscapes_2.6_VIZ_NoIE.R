...
u = merge(u, read.csv("genera.tsv", sep="\t"), by=c("Language", "Family"), all=TRUE)
...
sink("output/correlation-NoIE.txt")
print(cor.test(u$OSSameSide_Real_Prob, u$OSSameSide))
sink()
sink("output/correlation-spearman-NoIE.txt")
print(cor.test(u$OSSameSide_Real_Prob, u$OSSameSide, method="spearman"))
sink()
...
#ggsave("figures/fracion-optimized_DLM_2.6_format-NoIE.pdf", height=7, width=7)
#
