...
u = read.csv("landscapes_2.6_new.R.tsv", sep=" ") %>% filter(!(Language %in% c("Persian_2.6", "Turkish_2.6", "Faroese_2.6", "Icelandic_2.6")))
v = read.csv("landscapes_2.7_new.R.tsv", sep=" ")
u = rbind(u,v)
...
for(i in (1:nrow(u))) {
	...
	u$Language2[[i]] = str_replace(u$Language2[[i]], "_2.7", "")
...
ggsave("figures/fracion-optimized_DLM_2.7_format.pdf", height=7, width=7)
