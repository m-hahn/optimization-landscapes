...
library(lme4)
sink("output/landscapes_2.6_FuncHead_Cont.R.txt")
print(summary(lmer(OSSameSide_Real_Prob ~ OSSameSide + (1+OSSameSide_Prob|Family), data=u)))
...
library(ggplot2)
plot = ggplot(u, aes(x=OSSameSide_Real_Prob, y=OSSameSide_Prob, color=Family)) + geom_label(aes(label=Language)) + xlab("Fraction of SOV/VSO/OSV... Orders (Real)") + ylab("Fraction of SOV/VSO/OSV... Orders (DLM Optimized)")
...
ggsave("figures/fracion-optimized_DLM_2.6_FuncHead_Cont.pdf", height=13, width=13)
