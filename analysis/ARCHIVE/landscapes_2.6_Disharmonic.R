...
data = read.csv(paste(DEPS, "DLM_MEMORY_OPTIMIZED/locality_optimized_dlm/manual_output_funchead_fine_depl", "/", "auto-summary-lstm_2.6_full.tsv", sep=""), sep="\t")
dataBackup = data
...
dataS = data %>% filter(CoarseDependency == "obl")
...
realS = real %>% filter(Dependency == "obl")
