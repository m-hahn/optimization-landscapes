...
for(group in unique(us$Group)) { 
   plot = plot + geom_segment(data= u2s %>% filter(Group == group), aes(x=OSSameSide_Real_Prob_TRUE, xend= OSSameSide_Real_Prob_FALSE - 0.0 * sign(OSSameSide_Real_Prob_FALSE-OSSameSide_Real_Prob_TRUE), y=OSSameSide_TRUE, yend=OSSameSide_FALSE - 0.0 * sign(OSSameSide_FALSE-OSSameSide_TRUE)), arrow=arrow(length=unit(0.3, "cm"),type="closed"), size=0.8, color="orange")
#   plot = plot + geom_segment(data= u2s %>% filter(Group == group), aes(x=OSSameSide_Real_Prob_TRUE, xend=OSSameSide_Real_Prob_TRUE + 0.7*(OSSameSide_Real_Prob_FALSE-OSSameSide_Real_Prob_TRUE), y=OSSameSide_TRUE, yend=OSSameSide_TRUE + 0.7*(OSSameSide_FALSE- OSSameSide_TRUE)), arrow=arrow(length=unit(0.3, "cm"),type="closed"), size=0.8, color="orange")
   plot = plot + geom_point(data=unique(us %>% filter(Group == group) %>% dplyr::select(Group, Language, Time, OSSameSide_Real_Prob, OSSameSide)), aes(label=Time), color="orange", size=2)
   plot = plot + geom_label_repel(data=unique(us %>% filter(Group == group) %>% dplyr::select(Group, Language, Time, OSSameSide_Real_Prob, OSSameSide)), aes(label=Time), color="black", size=3)
}
...
plot = plot + theme( strip.text.x = element_text(        size = 15))
ggsave("../figures/historical_2.6_times_stationary_layout_new3.pdf", width=15, height=8)
