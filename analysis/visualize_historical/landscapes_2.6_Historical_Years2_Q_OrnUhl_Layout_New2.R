...
for(group in unique(us$Group)) { 
   plot = plot + geom_segment(data= u2s %>% filter(Group == group), aes(x=OSSameSide_Real_Prob_TRUE, xend= OSSameSide_Real_Prob_FALSE - 0.02 * sign(OSSameSide_Real_Prob_FALSE-OSSameSide_Real_Prob_TRUE), y=OSSameSide_TRUE, yend=OSSameSide_FALSE - 0.05 * sign(OSSameSide_FALSE-OSSameSide_TRUE)), arrow=arrow(length=unit(0.3, "cm"),type="closed"), size=0.8, color="orange")
#   plot = plot + geom_segment(data= u2s %>% filter(Group == group), aes(x=OSSameSide_Real_Prob_TRUE, xend=OSSameSide_Real_Prob_TRUE + 0.7*(OSSameSide_Real_Prob_FALSE-OSSameSide_Real_Prob_TRUE), y=OSSameSide_TRUE, yend=OSSameSide_TRUE + 0.7*(OSSameSide_FALSE- OSSameSide_TRUE)), arrow=arrow(length=unit(0.3, "cm"),type="closed"), size=0.8, color="orange")
   plot = plot + geom_label(data=us %>% filter(Group == group), aes(label=Time), color="blue", size=3)
}
...
ggsave("../figures/historical_2.6_times_stationary_layout_new2.pdf", width=15, height=8)
