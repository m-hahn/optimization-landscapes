...
relevantPath = "/u/scr/mhahn/deps/DLM_MEMORY_OPTIMIZED/locality_optimized_dlm/manual_output_funchead_fine_depl_icelandic/"
...
while True:
  ...
  sentsForThisRound = 0
  for sentence, metadata in corpus:
       try:
          year = int(metadata["sent_id"].split(".")[0])
       except ValueError:
          assert False, metadata
       if year < args.start or year >= args.end:
         continue
       sentsForThisRound += 1
       if sentsForThisRound % 100 == 0:
         print(sentsForThisRound, "sentences")
       if counter > 200000:
           print "Quitting at counter "+str(counter)
           quit()
       counter += 1
       printHere = (counter % 50 == 0)
       batchOrdered, overallLogprobSum = orderSentence(sentence, dhLogits, printHere)
       batchOrdered = [batchOrdered]

       if len(batchOrdered[0]) <= 2:   
         print "Skipping extremely short sentence"
         continue
       input_words = []
       input_pos_u = []
       input_pos_p = []
       for i in range(len(batchOrdered[0])+2):
       ...
       if True:
           ...
           lossesHead = [[Variable(torch.FloatTensor([0.0]))]*1 for i in range(len(batchOrdered[0])+1)]
       ...
       if counter % 10000 == 0:
          ...
          print "Saving"
          with open(TARGET_DIR+"/manual_output_funchead_fine_depl_icelandic/"+args.language+"-"+str(args.start)+"-"+str(args.end)+"_"+__file__+"_model_"+str(myID)+".tsv", "w") as outFile:
