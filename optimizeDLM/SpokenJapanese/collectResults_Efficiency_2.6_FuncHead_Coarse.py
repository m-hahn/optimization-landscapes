...
group = "DLM_MEMORY_OPTIMIZED/locality_optimized_dlm/manual_output_funchead_fine_depl_TuebaJS_Coarse"
   ...
   if "model" in filename:
      ...
      with open(PATH+filename, "r") as inFile:
          ...
          for line in inFile:
             ...
             if entry["CoarseDependency"] not in ["sbj", "comp"]:
                  continue
             print(entry)
...
with open(PATH+"auto-summary-lstm_2.6_coarse.tsv", "w") as outFile:
  print("\t".join(outHeader) , file=outFile)
  for entry in entries:
    print("\t".join([entry.get(x, "NA") for x in outHeader]), file=outFile)
print("WRITING", PATH+"auto-summary-lstm_2.6_coarse.tsv")
