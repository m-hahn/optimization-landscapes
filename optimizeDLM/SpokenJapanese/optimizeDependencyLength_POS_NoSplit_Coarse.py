...
parser.add_argument('--language', type=str, default="TuebaJS")
...
def initializeOrderTable():
   ...
   keysCounts = sorted(list(keysCounts.items()), key=lambda x:x[1])
   for x in keysCounts[-30:]:
     print(x)
...
relevantPath = "/u/scr/mhahn/deps/DLM_MEMORY_OPTIMIZED/locality_optimized_dlm/manual_output_funchead_fine_depl_TuebaJS_Coarse/"
  ...
  while True:
    ...
    for partition in partitions:
       ...
       if counter % 10000 == 0:
          ...
          print "Saving"
          with open(TARGET_DIR+"/manual_output_funchead_fine_depl_TuebaJS_Coarse/"+args.language+"_"+__file__+"_model_"+str(myID)+".tsv", "w") as outFile:
