...
orderBySentence = defaultdict(list)
...
fleissKappa /= len(orderBySentence) * len(files) * (len(files)-1)
fleissKappaExpected = (countP_**2 + countN_**2) / (countP_+countN_)**2
#quit()
...
for x in sentenceToHash[:10]:
    print(x)
    printSent(hashToSentence[x[0]])
for x in sentenceToHash[-10:]:
    print(x)
    printSent(hashToSentence[x[0]])
print(fleissKappa, fleissKappaExpected)
print(countP_ / (countP_+countN_))
