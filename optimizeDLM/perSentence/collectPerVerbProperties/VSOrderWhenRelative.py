...
from collections import defaultdict
matrix = {}
for x in [x+"_"+y for x in ["SV", "VS"] for y in ["Relative", "Root"]]:
   matrix[x] = 0
def mean(x):
   return sum(x)/(len(x)+1e-10)
for x in sentenceToHash:
  ...
  for word in sent:
   ...
   if word["posUni"] == "VERB":
    ...
    embeddingDirection = ("Root" if word["head"] == 0 else ("Relative" if word["dep"].startswith("acl") else "Other"))
    if len(subjects) > 0 and embeddingDirection != "Other":
