...
def orderSentence(sentence, dhLogits, printThings):
   ...
   if len(subjects_or_objects) > 0:
     encodings = [[x["dep"], x["posUni"], x["length"]] + ["@"+str(z) for z in flatten(sorted([(y["dep"]+"_"+str(int(math.sqrt(y["length"]-1))),) for y in sentence[x["head"]-1]["children"]]))] for x in subjects_or_objects]
