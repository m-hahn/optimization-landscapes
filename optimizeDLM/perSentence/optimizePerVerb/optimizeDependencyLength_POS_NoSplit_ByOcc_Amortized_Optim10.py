...
parser.add_argument('--lr_amortized', type=float, default=random.choice([0.0000001, 0.000001, 0.00001, 0.00002, 0.0001, 0.001, 0.01]))
...
def orderSentence(sentence, dhLogits, printThings):
   ...
   if len(subjects_or_objects) > 0:
     encodings = [[x["posUni"], x["length"]] + ["@"+str(z) for z in flatten(sorted([(y["dep"]+"_"+str(int(math.sqrt(y["length"]-1))),) for y in sentence[x["head"]-1]["children"]]))] for x in subjects_or_objects]
...
#           quit()
       ...
       loss += 0.001 * amortized_embeddings.weight.pow(2).mean()
       loss.backward()
       if printHere:
