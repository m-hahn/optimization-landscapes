...
def orderSentence(sentence, dhLogits, printThings, orderKeys):
   ...
   for line in sentence:
      ...
      if printThings: 
         print "\t".join(map(str,["ORD", line["index"], (line["word"]+"           ")[:10], (".".join(list(key)) + "         ")[:22], line["head"], dhSampled, direction, (str(float(0))+"      ")[:8], str(1/(1+exp(-dhLogits[key])))[:8], (str(distanceWeights[stoi_deps[key]].data.numpy())+"    ")[:8] , str(originalDistanceWeights[key])[:8]    ]  ))
   ...
   #    if len(sentence) <= 2:
    #      print("Skipping short sentence")
       ...
       for subject in subjects:
         subjectItem = [x for x in batchOrdered if x["dep"] == subject]
#         print(subjectItem)
         if len(subjectItem) == 0: # Sometimes, a subject depends on punctuation
             continue
         assert len(subjectItem) == 1, (subject, current[0])
         realOrder = (1 if subjectItem[0]["index"] < subjectItem[0]["head"] else -1)
  #       print("SUBJECT", subject, counter/float(len(corpus)), realOrder, depLengthReal)
...
#           print("          Dependency length", best, sum(depLengths)/(0.001+len(depLengths)), sd(depLengths)/sqrt(100))
         ...
         if best1[1][-1] > best1[-1][-1] + 2 and realOrder == 1:
             for line in current[0]:
                 print("\t".join([str(line["index"]),line["word"],  str(line["head"]), str(line["dep"]), "-----" if line["dep"] == subject else ""]))
             print(bestResult[-1])
             print(bestResult[1])
         print >> outFile, subject +"\t"+str(lengthsPerOrder[1] - lengthsPerOrder[-1]) +"\t"+str(bestLengthsPerOrder[1] - bestLengthsPerOrder[-1]) + "\t"+ str(bestLengthsPerOrder[1]) + "\t" + str(lengthsPerOrder[1]) + "\t"+ str(depLengthReal) + "\t" + str(realOrder)
       if counter % 10 == 0:
