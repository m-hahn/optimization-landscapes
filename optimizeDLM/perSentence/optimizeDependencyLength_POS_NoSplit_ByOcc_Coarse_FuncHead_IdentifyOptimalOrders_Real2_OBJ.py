...
def orderChildrenRelative(sentence, remainingChildren, reverseSoftmax, orderKeys):
       ...
       while len(remainingChildren) > 0:
           ...
           if sentence[remainingChildren[selected]-1]["dependency_key"] == orderKeys["object_"]:
...
objectsPerSentence = defaultdict(list)
for dep in stoi_deps:
   if dep.startswith("obj_"):
       _, sentHash, _ = dep.split("_")
       objectsPerSentence[sentHash].append(dep)
...
with open(PATH, "w") as outFile:
  ...
  for sentence in corpus:
       ...
       sentenceHash = hash_(" ".join([x["word"] for x in sentence]))
       objects = objectsPerSentence[sentenceHash]
       batchOrdered, overallLogprobSum = orderSentence(current[0], dhLogits, printHere, {"object_" : "NONE", "order" : "NONE", "key" : "NONE"})
       depLengthReal = getDependencyLength(batchOrdered)
       for object_ in objects:
         print("SUBJECT", object_, counter/float(len(corpus)))
         ...
         for order in [-1, 1]:
           ...
           print("    ORDER", order)
           for key in range(10):
               batchOrdered, overallLogprobSum = orderSentence(current[0], dhLogits, printHere, {"object_" : object_, "order" : order, "key" : key})
         ...
         print >> outFile, object_ +"\t"+str(lengthsPerOrder[1] - lengthsPerOrder[-1]) +"\t"+str(bestLengthsPerOrder[1] - bestLengthsPerOrder[-1]) + "\t"+ str(bestLengthsPerOrder[1]) + "\t" + str(lengthsPerOrder[1]) + "\t"+ str(depLengthReal)
