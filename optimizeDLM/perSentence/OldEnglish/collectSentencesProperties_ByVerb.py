...
for x in sentenceToHash:
 #   print(x)
    ...
    subjects = [i for i in range(len(sent)) if sent[i]["dep"] == "sub" and sent[sent[i]["head"]-1]["posUni"] == "V"]
    objects = [i for i in range(len(sent)) if sent[i]["dep"] == "obj" and sent[sent[i]["head"]-1]["posUni"] == "V"]
