...
for filepath in files:
  ...
  with open(filepath, "r") as inFile:
   ...
   if True:
    with open("/u/scr/mhahn/deps/DLM_MEMORY_OPTIMIZED/locality_optimized_dlm/manual_output_funchead_fine_depl_perSent_perOcc_coarse_funchead_SUBJ_DLM/optimizeDependencyLength_POS_NoSplit_ByOcc_Coarse_FuncHead_IdentifyOptimalOrders_Best_ByGroup_SO.py_"+language+"_"+model, "r") as inFile:
...
#      quit()
      ...
      for bonused in orders:
        for OS_Penalty in [0.0]: #, 1.0, 2.0, 3.0]:
          ...
          variablesByX = []
          orderByVar = []
          cachedPenaltyPerOrder = { order: (-0.1 if order == bonused else 0.0) for order in orders}
