...
parser.add_argument('--lr_amortized', type=float, default=random.choice([0.00001, 0.00002, 0.0001, 0.0002, 0.001, 0.001, 0.01, 0.1]))
...
def orderSentence(sentence, dhLogits, printThings):
   ...
   if len(subjects_or_objects) > 0:
     encodings = [[x["dep"]+"_"+x["posUni"], x["dep"]+"_"+str(x["length"])] + [x["dep"]+"_"+"@"+str(z) for z in flatten(sorted([(y["dep"]+"_"+str(int(math.sqrt(y["length"]-1))),) for y in sentence[x["head"]-1]["children"] if x != y]))] for x in subjects_or_objects]
...
amortized_embeddings = torch.nn.Embedding(300, 2)
...
#           quit()
  ...
  for i in range(len(stoi_encodings_)-1, -1, -1):
     print(stoi_encodings_[i], [float(x) for x in amortized_embeddings.weight[i]])
