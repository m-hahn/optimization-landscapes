...
#   print(children_categories)
   if True:
      subjects = [i for i in range(len(children_categories)) if object_(children_categories[i]) and not isAPronoun(tree["children"][i]) and length(tree["children"][i]) >= 1]
...
#         print(tree["category"], children_categories, verbs)
         ...
         matrix["objects"].append((len([i for i in range(len(children_categories)) if subject(children_categories[i]) and not isAPronoun(tree["children"][i]) and length(tree["children"][i]) >= 1])))
