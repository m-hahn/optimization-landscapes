...
parser.add_argument('--lr_grammar', type=float, default=random.choice([0.0001, 0.0002, 0.001, 0.001, 0.001, 0.005]))
parser.add_argument('--momentum_grammar', type=float, default=random.choice([0.0, 0.2, 0.8, 0.9]))
parser.add_argument('--lr_amortized', type=float, default=random.choice([0.00001, 0.0001, 0.001, 0.001, 0.01, 0.1]))
parser.add_argument('--momentum_amortized', type=float, default=random.choice([0.0, 0.2, 0.8, 0.9]))
parser.add_argument('--weight_decay', type=float, default=random.choice([0.000001, 0.00001, 0.0001, 0.001, 0.01, 0.1]))
...
def orderSentence(sentence, dhLogits, printThings):
   ...
   if len(subjects_or_objects) > 0:
     encodings = [[x["dep"]+"_"+x["posUni"], x["dep"]+"_"+x["posUni"]+"_"+str(x["length"]), x["dep"]+"_"+x["posUni"]+"_"+sentence[x["head"]-1]["dep"]] + [x["dep"]+"_"+"@"+str(z) for z in flatten(sorted([(y["dep"]+"_"+str(int(math.sqrt(y["length"]-1))),) for y in sentence[x["head"]-1]["children"] if x != y]))] for x in subjects_or_objects]
...
amortized_embeddings = torch.nn.Embedding(1200, 2)
...
#           quit()
       ...
       loss += args.weight_decay * amortized_embeddings.weight.pow(2).mean()
       loss.backward()
       if printHere:
         print(args)
       ...
       if "PAD" in itos_encodings_:
          amortized_embeddings.weight.data[itos_encodings_["PAD"]].zero_()
