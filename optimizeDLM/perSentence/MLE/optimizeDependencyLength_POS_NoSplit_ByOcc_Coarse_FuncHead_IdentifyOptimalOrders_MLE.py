...
with open("/u/scr/mhahn/deps/LANDSCAPE/mle-fine_selected/auto-summary-lstm_2.6.tsv", "r") as inFile:
   header = next(inFile).strip().split("\t")
   print(header)
   header = dict(list(zip(header, range(len(header)))))
   for line in inFile:
     line = line.strip().split("\t")
     if line[header["Language"]] == args.language:
      coarse_dep = line[header["Dependency"]]
      if coarse_dep == "nsubj":
          continue
      dhWeights.data[stoi_deps[coarse_dep]] = 100*float(line[header["DH_Mean_NoPunct"]])
      distanceWeights.data[stoi_deps[coarse_dep]] = 100*float(line[header["Distance_Mean_NoPunct"]])
...
with open(PATH, "w") as outFile:
  ...
  for sentence in corpus:
       ...
       for subject in subjects:
         ...
         print >> outFile, subject +"\t"+str(lengthsPerOrder[1] - lengthsPerOrder[-1]) +"\t"+str(bestLengthsPerOrder[1] - bestLengthsPerOrder[-1])
