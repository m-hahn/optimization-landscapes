...
def descent(tree, time):
   ...
   subjects = [i for i in range(len(children_categories)) if children_categories[i].startswith("NP-OB") and not isAPronoun(tree["children"][i]) and length(tree["children"][i]) >= 1]
   if len(subjects) > 0:
...
#         print(tree["category"], children_categories, verbs)
         ...
         matrix["objects"].append((len([i for i in range(len(children_categories)) if children_categories[i].startswith("NP-SBJ") and not isAPronoun(tree["children"][i]) and length(tree["children"][i]) >= 1])))
...
summary(lm(time ~ isRoot * objects + isRoot * subjectLength + isRoot * verbDependents + isRoot * verbLength, data=data))
