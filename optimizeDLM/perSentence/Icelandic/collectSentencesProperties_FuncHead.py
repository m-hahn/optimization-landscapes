...
#         print(tree["category"], children_categories, verbs)
         ...
         matrix["objects"].append((len([i for i in range(len(children_categories)) if children_categories[i].startswith("NP-OB") and not isAPronoun(tree["children"][i]) and length(tree["children"][i]) >= 1])))
