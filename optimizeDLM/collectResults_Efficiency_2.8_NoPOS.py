...
group = "DLM_MEMORY_OPTIMIZED/locality_optimized_dlm/manual_output_funchead_fine_depl_nopos"
...
#      print("READING "+filename )
      ...
      with open(PATH+filename, "r") as inFile:
          ...
          for line in inFile:
             ...
             if not ("_2.8_" in filename):
               collectedFilesNumber-=1
               break
             entry["Language_"] = entry["Language"].replace("_2.8", "")
             if "FileName" not in header:
                entry["FileName"] = filename[filename.rfind("_")+1:-4]
             if entry["CoarseDependency"] not in ["obj", "nsubj"]:
                  continue
...
with open(PATH+"auto-summary-lstm_2.8.tsv", "w") as outFile:
  print("\t".join(outHeader) , file=outFile)
  for entry in entries:
    print("\t".join([entry.get(x, "NA") for x in outHeader]), file=outFile)
print("WRITING", PATH+"auto-summary-lstm_2.8.tsv")
