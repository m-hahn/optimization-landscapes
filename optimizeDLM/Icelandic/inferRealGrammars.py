...
parser.add_argument('--language', type=str, default="Icelandic-IcePaHC")
...
def orderSentence(sentence, dhLogits, printThings, dhWeights, distanceWeights):
   ...
   for line in sentence:
      line["linearization_logprobability"]       = None                      
      line["children_HD"]                       = None
      line["children_DH"] = None
      line["children_decisions_logprobs"] = None
      line["ordering_decision_log_probability"] =None
      del line["linearization_logprobability"]                             
      del line["children_HD"]
      del line["children_DH"]
      del line["children_decisions_logprobs"]
      del line["ordering_decision_log_probability"]
   for line in sentence:
      line["coarse_dep"] = makeCoarse(line["dep"])
      if line["coarse_dep"] == "root":
          root = line["index"]
...
def inRange(x):
   return x >= args.start and x < args.end

counter = 0
corpus = [x for x, metadata in list(CorpusIterator(args.language,"together").iterator()) if inRange(int(metadata["sent_id"].split(".")[0]))]
...
def model(corpus):
  ...
  for q in pyro.irange("data_loop", len(corpus), subsample_size=5, use_cuda=False):
       point = corpus[q]
       current = [point]
       counter += 1
       ...
       if printHere:
         print "BACKWARD 3 "+__file__+" "+args.language+" "+str(myID)+" "+str(counter), args.start, args.end
...
for step in range(1,n_steps):
    ...
    if step % 2000 == 0:
       print "Saving"
       with open("/u/scr/mhahn/deps/LANDSCAPE/mle-fine-icelandic/"+args.language+"-"+str(args.start)+"-"+str(args.end)+"_"+__file__+"_model_"+str(myID)+".tsv", "w") as outFile:
