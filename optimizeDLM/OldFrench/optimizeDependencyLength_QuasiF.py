...
from corpusIterator_Hybrid import CorpusIteratorHybrid as CorpusIterator_V
...
def orderSentence(sentence, dhLogits, printThings):
   ...
   for line in sentence:
      ...
      if printThings: 
         print("\t".join(list(map(str,["ORD", line["index"], (line["word"]+"           ")[:10], ("".join(list(key)) + "         ")[:22], line["head"],line["func_head"], dhSampled, direction, (str(probability.data.numpy())+"      ")[:8], (str(distanceWeights_[stoi_deps[key]].data.numpy())+"    ")[:8]  ]  ))))
...
dhWeights.data[:, stoi_deps["case"]] = 10.0
dhWeights.data[:, stoi_deps["acl"]] = -10.0
dhWeights.data[:, stoi_deps["mark"]] = 10.0
