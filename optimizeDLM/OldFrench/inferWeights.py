...
#torch.exp(line["ordering_decision_log_probability"]).data.numpy()[0],
      if printThings: 
         print("\t".join(list(map(str,["ORD", line["index"], (line["word"]+"           ")[:10], ("".join(list(key)) + "         ")[:22], line["head"], dhSampled, direction, (str(probability.data.numpy())+"      ")[:8], str(1/(1+exp(-dhLogits[key])))[:8], (str(distanceWeights[stoi_deps[key]].data.numpy())+"    ")    ]  ))))
...
def model(corpus):
  ...
  for q in pyro.irange("data_loop", corpus.length(), subsample_size=5, use_cuda=False, device="cpu"):
...
#       print [name, pyro.param(name).data.numpy()]
    ...
    if step % 2000 == 0:
       ...
       with open(save_path+"/manual_output_ground_coarse/"+args.language+"_"+__file__+"_model_"+str(myID)+".tsv", "w") as outFile:
          print("\t".join(list(map(str,["Counter", "DH_Mean_NoPunct", "DH_Sigma_NoPunct", "Distance_Mean_NoPunct", "Distance_Sigma_NoPunct", "Dependency"]))), file=outFile)
          ...
          for i in range(len(itos_deps)):
             key = itos_deps[i]
             dependency = key
             print("\t".join(list(map(str,[counter, dh_numpy[i], dh_sigma_numpy[i], dist_numpy[i], dist_sigma_numpy[i], dependency]))), file=outFile)
