...
#       print [name, pyro.param(name).data.numpy()]
    ...
    if step % 2000 == 0:
       ...
       with open(save_path+"/manual_output_ground_coarse/"+args.language+"_"+__file__+"_model_"+str(myID)+".tsv", "w") as outFile:
          print("\t".join(list(map(str,["Counter", "Document", "DH_Mean_NoPunct", "DH_Sigma_NoPunct", "Distance_Mean_NoPunct", "Distance_Sigma_NoPunct", "Dependency"]))), file=outFile)
          ...
          for i in range(len(itos_deps)):
             ...
             for doc in range(len(itos_docs)):
                print("\t".join(list(map(str,[counter, itos_docs[doc], dh_numpy[doc,i], dh_sigma_numpy[doc,i], dist_numpy[doc,i], dist_sigma_numpy[doc,i], dependency]))), file=outFile)
