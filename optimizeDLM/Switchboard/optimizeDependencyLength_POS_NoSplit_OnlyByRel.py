...
with open("ptb2uni.txt", "r") as inFile:
   ptb2uni = dict([x.split("\t") for x in inFile.read().strip().split("\n")])
for x in list(ptb2uni):
   ptb2uni["^"+x] = ptb2uni[x]
ptb2uni["^VB^RP"] = ptb2uni["VB"]
ptb2uni["^NNP^POS"] = ptb2uni["NNP"]
ptb2uni["^CD^NN"] = ptb2uni["NN"]
ptb2uni["NNS^POS"] = ptb2uni["NNS"]
ptb2uni["^NN^NNS"] = ptb2uni["NN"]
ptb2uni["^DT^NN"] = ptb2uni["NN"]
ptb2uni["^PTP^VBP"] = ptb2uni["VBP"]
ptb2uni["^VBP^RB"] = ptb2uni["VBP"]
ptb2uni["^IN^IN"] = ptb2uni["IN"]
ptb2uni["^IN^NN"] = ptb2uni["NN"]
ptb2uni["TO|IN"] = ptb2uni["IN"]
import random
import sys
...
parser.add_argument('--language', type=str, default="English_SWBD")
parser.add_argument('--entropy_weight', type=float, default=0.001)
parser.add_argument('--lr_policy', type=float, default=random.choice([0.001, 0.01, 0.1]))
...
def processPOS(pos):
   if pos not in ptb2uni:
      if "|" in pos:
        posHead0_ = [ptb2uni[x] for x in pos.split("|") if x != "UH"]
        if len(set(posHead0_)) == 1:
            ptb2uni[pos] = posHead0_[0]
            return
      elif "^" in pos:
        posHead0_ = [ptb2uni[x] for x in pos.split("^") if len(x) > 0]
        if "NOUN" in posHead0_ and "VERB" not in posHead0_:
            ptb2uni[pos] = "NOUN"
            return
        if "NOUN" not in posHead0_ and "VERB" in posHead0_:
            ptb2uni[pos] = "VERB"
            return
   ...
   for partition in ["together"]:
     for sentence in CorpusIterator(args.language,partition).iterator():
      for line in sentence:
          ...
          #print(line)
          processPOS(line["posUni"])
          posUni.add(ptb2uni[line["posUni"]])
  
          if line["head"] == None:
             continue
          if line["fine_dep"] == "root":
             continue
          posHere = ptb2uni[line["posUni"]]
          if sentence[line["head"]-1]["posUni"] == "-DFL-":
 #            print("Head is DFL!", line)
             continue
          if sentence[line["head"]-1]["posUni"] == "XX":
#             print("Head is XX!", line)
             continue
          posHead0 = sentence[line["head"]-1]["posUni"]
          processPOS(posHead0)
          posHead = ptb2uni[sentence[line["head"]-1]["posUni"]]
          dep = line["fine_dep"]
          direction = "HD" if line["head"] < line["index"] else "DH"
          key = dep #(posHead, dep, posHere)
...
itos_deps = sorted(vocab_deps, key=lambda x:x)
stoi_deps = dict(zip(itos_deps, range(len(itos_deps))))

print itos_deps


relevantPath = "/u/scr/mhahn/deps/DLM_MEMORY_OPTIMIZED/locality_optimized_dlm/manual_output_funchead_fine_depl_PTB/"
...
#         print "Skipping extremely short sentence"
       ...
       for i in range(maxLength+2):
          ...
          input_pos_u.append(map(lambda x: 2 if i == 0 else (stoi_pos_uni[ptb2uni[x[i-1]["posUni"]]]+3 if i <= len(x) else 0), batchOrdered))
       ...
       if counter % 10000 == 0:
          ...
          print "Saving"
          with open(TARGET_DIR+"/manual_output_funchead_fine_depl_PTB/"+args.language+"_"+__file__+"_model_"+str(myID)+".tsv", "w") as outFile:
             print >> outFile, "\t".join(map(str,["DH_Weight","CoarseDependency","DistanceWeight", "Language", "FileName"]))
             for i in range(len(itos_deps)):
                rel = itos_deps[i]
                dhWeight = dhWeights[i].data.numpy()
                distanceWeight = distanceWeights[i].data.numpy()
                print >> outFile, "\t".join(map(str,[round(dhWeight, 5), rel, round(distanceWeight, 5), args.language, myID]))
