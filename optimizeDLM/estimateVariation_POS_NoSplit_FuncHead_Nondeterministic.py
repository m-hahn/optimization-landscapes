...
with open("/u/scr/mhahn/TMP.R", "w") as outFile:
  print >> outFile, ("lengths = c("+",".join(map(str, dependencyLengths))+")")

def mean(x):
   return sum(x)/(len(x)+0.0000001)

subsamplingIndex = int(sqrt(len(dependencyLengths)))
randomState = Random(0)
with open("/u/scr/mhahn/TMPN_"+args.language, "w") as outFile:
 key = (head, rel, dep)
 i = stoi_deps[key]
 ...
 for i in range(1000):
   randomState.shuffle(dependencyLengths)
   print >> outFile, (mean(dependencyLengths[:subsamplingIndex]))
