...
def readUDCorpus(language, partition, ignoreCorporaWithoutWords=True, Partition=None):
      ...
      files = sorted(list(filter(lambda x:x.startswith("UD_"+language.replace("-Adap", "")), files)))
...
#        print >> sys.stderr, ("SUBDIR FILES", subDirFiles)
      ...
      data = [data[i] for i in range(len(data)) if i % 2 == Partition]
...
class CorpusIterator_V():
   def __init__(self, language, partition="together", storeMorph=False, splitLemmas=False, shuffleData=True, shuffleDataSeed=None, splitWords=False, ignoreCorporaWithoutWords=True, Partition=None):
      ...
      if language.startswith("ISWOC_"):
         ...
         assert len(data) > 0, (language, partition)
          ...
          data = readUDCorpus(language, partition, ignoreCorporaWithoutWords=ignoreCorporaWithoutWords, Partition=Partition)
      if shuffleData:
