...
def initializeOrderTable():
   ...
   for partition in ["together"]:
     for sentence in CorpusIterator(language,partition, Partition=Partition).iterator():
...
counter = 0
corpus = CorpusIterator(language,"together", Partition=Partition)
...
for step in range(1,n_steps):
    ...
    if step % 2000 == 0:
       print "Saving"
       with open("/u/scr/mhahn/deps/LANDSCAPE/mle-fine_disjoint/"+language+str(Partition)+"_"+__file__+"_model_"+str(myID)+".tsv", "w") as outFile:
