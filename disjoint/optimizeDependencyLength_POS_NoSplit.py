...
def initializeOrderTable():
   ...
   for partition in ["together"]:
     for sentence in CorpusIterator(args.language,partition, Partition=args.Partition).iterator():
...
relevantPath = "/u/scr/mhahn/deps/DLM_MEMORY_OPTIMIZED/locality_optimized_dlm/manual_output_funchead_fine_depl_disjoint/"

import os
files = [x for x in os.listdir(relevantPath) if x.startswith(args.language+str(args.Partition)+"_") and __file__ in x]
posCount = 0
negCount = 0
for name in files:
...
counter = 0
while True:
  corpus = CorpusIterator(args.language, Partition=args.Partition).iterator(rejectShortSentences = True)
  ...
  while True:
    ...
    for partition in partitions:
       ...
       if counter % 10000 == 0:
          ...
          print "Saving"
          with open(TARGET_DIR+"/manual_output_funchead_fine_depl_disjoint/"+args.language+str(args.Partition)+"_"+__file__+"_model_"+str(myID)+".tsv", "w") as outFile:
